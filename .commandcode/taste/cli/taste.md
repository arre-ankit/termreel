...
- Store CLI config (API keys, preferences) in `~/.config/<cli-name>/config.json`. Confidence: 0.70
- When bundling CLIs, inline JSON data files as static imports to avoid `__dirname` path issues in the bundle. Confidence: 0.70
- When integrating AI providers, build a provider abstraction layer supporting multiple providers (e.g., Anthropic + OpenAI) with user-selectable preference stored in config. Confidence: 0.70
